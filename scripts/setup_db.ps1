...
.DESCRIPTION
  Applies the local Supabase shim, then every migration in order, into a fresh
  database on a local PostgreSQL server. Each file runs under --single-transaction,
  exactly as production is migrated; 0049 onward refuses any other executor. Nothing outside this repository is
  needed: the production-specific preflights in 0019c-0030 are skipped under
...
.PARAMETER UpTo      last migration number to apply, e.g. '0048' (default: all)
.PARAMETER Port      PostgreSQL port (default 55433)
.PARAMETER PgBin     directory holding psql.exe (default: PGBIN env, then PATH)

.EXAMPLE
  pwsh -File scripts/setup_db.ps1 -Database mm -UpTo 0051
#>
[CmdletBinding()]
param(
...
function Run-Sql {
    ...
    $out = & $psql @H -d $Database -v ON_ERROR_STOP=1 --single-transaction -q -f $File 2>&1 | Out-String
    if ($LASTEXITCODE -ne 0 -or $out -cmatch '(?m)^(psql:.*)?ERROR:') {
        Write-Host "FAIL $(Split-Path $File -Leaf)" -ForegroundColor Red
        ($out -split "`r?`n" | Where-Object { $_ -match 'ERROR|DETAIL|HINT' } | Select-Object -First 8) | ForEach-Object { Write-Host "  $_" }
        exit 1
    }
}
...
# The fresh-install flag is a database-level setting so every later session and
# every psql -f sees it without each file having to be edited.
& $psql @H -d $Database -q -c "alter database $Database set mm.fresh_install = 'on';" | Out-Null

$applied = 0
foreach ($f in Get-ChildItem (Join-Path $repo 'migrations') -File -Filter '*.sql' | Sort-Object Name) {
    $num = ($f.Name -split '_')[0] -replace '[a-z]$', ''
    if ($num -gt $UpTo) { break }
    Run-Sql $Database $f.FullName
    $applied++
}
Write-Host "OK: $applied migration(s) applied to $Database (fresh install, up to $UpTo)"
