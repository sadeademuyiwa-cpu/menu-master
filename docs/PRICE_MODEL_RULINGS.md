...
| **2** | **D-1 — DIRECTION APPROVED; implementation-ready design awaiting sign-off.** `docs/D1_SCHEDULER_DESIGN.md`. Two invariants carry it: **due-or-overdue predicates** (a missed run self-recovers, no catch-up mode) and **derive, don't decide** (jobs stamp and send; they never own entitlement). Provider commands dispatch **at request time**, not at the boundary — otherwise a scheduler outage would let the old plan renew at the old price. | **`0036`** | Remaining gate is **V-1**, one read-only query. |
