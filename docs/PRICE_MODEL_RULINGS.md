...
| **2** | **D-1 — DESIGN PROPOSED, awaiting approval.** `docs/D1_SCHEDULER_DESIGN.md`: `pg_cron` as the engine, a scheduled Edge Function as the only outbound arm, an outbox between them. Governing principle: **lateness must never grant or remove entitlement** — everything derivable stays derived, so a stopped scheduler delays audit and notification without ever being wrong. | **`0033`** and every deadline | External schedulers rejected on security grounds; the engine is swappable because every job is a SQL function. |
